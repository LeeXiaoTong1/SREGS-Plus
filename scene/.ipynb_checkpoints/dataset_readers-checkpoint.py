...
#     # ply_path = os.path.join(path, "sparse/0/points3D.ply")
#     # bin_path = os.path.join(path, "sparse/0/points3D.bin")
#     ply_path = os.path.join(path, str(n_views) + "_views/triangulated/points3D.txt")
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            # 去掉路径重复拼接，只保留 'path' 和 'frame["file_path"]'
            ...
            mask = norm_data[:, :, 3:4]
            # 假设 bounds 为 [0.1, 10.0]
            bounds = np.array([0.1, 10.0], dtype=np.float64)  # 确保 bounds 是 float64 类型
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path,
                                        image_name=image_name, width=image.size[0], height=image.size[1], mask=mask, bounds=bounds))
    return cam_infos



def readNerfSyntheticInfo(path, white_background, eval, rand_pcd, n_views=0, extension=".png"):
