...
This module is intentionally conservative: Qwen/VLM is used only for mining
failure regions in pseudo views. The actual training response is implemented at
Gaussian parameter level and gated by the mined masks.
"""
...
class VLASREGSController:
    ...
    The controller keeps the VLM mining path and the differentiable Gaussian
    update path separated:
      - scheduled mining: render pseudo view, call Qwen, map boxes to Gaussian ids;
      - every iteration: apply weak regularizers and gradient modulation to the
        currently active Gaussian masks.
    """
    ...
    def apply_gradient_modulation(self, iteration: int, gaussians) -> None:
        ...
        # Geometry ambiguity: suppress opacity growth and unstable scale update.
        if geo.any():
            if getattr(gaussians, "_opacity", None) is not None and gaussians._opacity.grad is not None:
                g = gaussians._opacity.grad
                # Negative opacity-gradient increases opacity after Adam step. Scale only that direction.
                neg = g[geo] < 0
                g_geo = g[geo]
        ...
        # Appearance collapse: reduce high-order SH memorization on suspicious Gaussians.
        if app.any():
    ...
    def mine_once(self, iteration: int, gaussians) -> Dict[str, float]:
        with torch.no_grad():
            pseudo_cam, pseudo_score = self._select_pseudo_camera(gaussians)
            ...
            regions = self._invoke_qwen(rgb_path, panel_path, qwen_out_path)
            ...
            geo_mask, app_mask = self._regions_to_gaussian_masks(
                regions=regions,
                camera=pseudo_cam,
                gaussians=gaussians,
            ...
            info = {
                "vla/selected": 1.0,
                "vla/pseudo_inb": float(pseudo_score),
            ...
            print(
                f"[VLA@{iteration}] pseudo_inb={pseudo_score:.3f} "
                f"regions={len(regions)} geo_g={int(geo_mask.sum())} app_g={int(app_mask.sum())}"
            )
            return info
    ...
    def _select_pseudo_camera(self, gaussians):
        ...
        candidates = [self._pseudo_stack.pop() for _ in range(n_try)]
        ...
        for cam in candidates:
            score = self._camera_inbound_ratio(cam, gaussians)
            scored.append((score, cam))

        valid = [(s, c) for s, c in scored if self.inb_min <= s <= self.inb_max]
        if valid:
            # pick the most novel but still valid pseudo view
            score, cam = sorted(valid, key=lambda x: x[0])[0]
            return cam, score

        above = [(s, c) for s, c in scored if s >= self.inb_min]
        if above:
            score, cam = sorted(above, key=lambda x: x[0])[0]
            return cam, score

        score, cam = sorted(scored, key=lambda x: abs(x[0] - self.inb_min))[0]
        return cam, score
    ...
    def _camera_inbound_ratio(self, camera, gaussians) -> float:
        ...
        if xyz.shape[0] > self.score_num_points:
            ...
            xyz = xyz[ids]
        px, py, depth, valid = self._project_xyz(camera, xyz)
    ...
    def _invoke_qwen(self, rgb_path: str, panel_path: str, out_path: str) -> List[VLARegion]:
        ...
        try:
            ...
            if self.require_qwen:
                raise
            print(f"[VLA] Qwen call failed: {exc}")
            return []

        if proc.returncode != 0:
            msg = (proc.stderr or proc.stdout or "").strip()
            if self.require_qwen:
                raise RuntimeError(f"Qwen worker failed with code {proc.returncode}: {msg}")
            print(f"[VLA] Qwen worker failed, skip this mining step: {msg[:300]}")
            return []
    ...
    def _fallback_regions(self, render_pkg, camera) -> List[VLARegion]:
        ...
        if low.float().mean() < 0.01:
            return []
        # grid-level boxes: avoids connected-component dependency.
        gh, gw = 8, 8
