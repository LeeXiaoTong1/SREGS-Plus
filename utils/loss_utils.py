...
def patch_norm_mse_loss(input, target, patch_size, margin, return_mask=False):
    ...
    # 对齐数量：取最小的那个
    n = min(input_patches.shape[0], target_patches.shape[0])
    input_patches  = input_patches[:n]
    target_patches = target_patches[:n]
...
def masked_robust_norm(x: torch.Tensor, mask: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """
    x: [H,W]
    mask: [H,W] bool
    用 median/MAD 做鲁棒归一化；统计量用 detach，避免不稳定梯度。
    """
    if mask.sum() < 64:
        return x * 0.0  # 防止极端情况炸梯度
    vals = x[mask].detach()
    med = vals.median()
    mad = (vals - med).abs().median()
    return (x - med) / (mad + eps)

def masked_pearson(x: torch.Tensor, y: torch.Tensor, mask: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """
    返回相关系数（标量），可反传到 x,y。
    """
    if mask.sum() < 64:
    ...
    xv = xv - xv.mean()
    yv = yv - yv.mean()
    denom = xv.std(unbiased=False) * yv.std(unbiased=False) + eps
