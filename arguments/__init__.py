...
def get_n_views_from_argv(default=0):
    argv = sys.argv[1:]

    for i, a in enumerate(argv):
        if a == "--n_views" and i + 1 < len(argv):
            try:
                return int(argv[i + 1])
            except Exception:
                return default

        if a.startswith("--n_views="):
            try:
...
class OptimizationParams(ParamGroup):
    def __init__(self, parser):
        n_views = get_n_views_from_argv(default=0)
